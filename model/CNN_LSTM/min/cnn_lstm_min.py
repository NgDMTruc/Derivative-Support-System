...
data = pd.read_csv('final_min.csv')
...
while unique_trials > len(set(str(t.params) for t in study.trials)):
    ...
    #     print('Trade times in sample:',i)
        ...
        study.trials_dataframe().fillna(0).sort_values('values_0').to_csv('cnn_lstm_feature_trials.csv')

        joblib.dump(study, 'cnn_lstm_model.pkl')
    except:
        continue
...
for cluster_number, cluster_indices in clstrsNew.items():
    ...
    # top_10_features_cluster = [feature for feature, _ in sorted_cluster_frequency]
