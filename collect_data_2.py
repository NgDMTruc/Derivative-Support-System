...
from data.data_utils import get_vn30f, add_features, add_finance_features,get_vn30f_current
import sys
...
def get_latest_time(table_name, user, password, host, port, dbname, schema):
    ...
    last_time = datetime.strptime(last_time_str, '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone(timedelta(hours=7)))
...
def main():
    ...
    user_input ='2024-11-25 10:30:00'
 
    
    now = datetime.strptime(user_input, '%Y-%m-%d %H:%M:%S')
    ...
    for i in range(len(db_data)):
        last_time, _ = get_latest_time(db_data[i], user, password, host, port, dbname, schema)
       
        if now > last_time:
           
            new_data = get_vn30f_current(symbol, resolution[i] ,str(last_time)[:19], str(now)[:19])
            
            append_to_postgresql(new_data, db_data[i], user, password, host, port, dbname)
